...
class seird_misfit:
    def __init__(self, infected_cases, deceased_cases, date, simulation_time, validation = False, validation_day = 10, truncated_gaussian = False):
        
        self.noise_variance = None
        self.truncated_gaussian = truncated_gaussian
        self._validation_days = validation_day
    ...
    def cost(self, x):
        ...
        if self._validation:
            return np.sum(np.divide(np.power(self.data[-self._validation_days:,:] - x[STATE][-self._validation_days:,:], 2), 2*self.noise_variance[-self._validation_days:,:]))
        else:
